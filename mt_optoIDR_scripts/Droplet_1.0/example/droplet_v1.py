    ...
    for j in range(len(files)):
        ...
        for k in channels:
            ...
            if len(filedata.shape) == 6:
                image = filedata[f1-1,int(k),int(z1-1),:,:,0]
            ...
            for l in frames:
                ...
                for m in zstack:                
                    ...
                    # creates the proper format for each frame of the image
                    if len(filedata.shape) == 6:
                        image = filedata[int(l),int(k),m,:,:,0]
                    elif len(filedata.shape) == 8:
                        image = filedata[int(l),0,int(k),0,m,:,:,0]
                    ...
                    for i in range(len(centroids[:,1])):
                        d3 = {'x-centroids': centroids[i,1], 'y-centroids': centroids[i,0], 
                              ...
                              'puncta intensity': punctInt[0,i], 'partition coeff': partCoef[0,i],
                              'file number': 'file' + str(j+1),'channel number': 'channel' + str(k+1),
                              'frame number': 'frame' + str(l+1), 'stack number': 'zstack' + str(m+1),
                              'puncta index': str(i), 'covariances': pcovArray[i,:]}
...
o, n, m, l, q = condensate(r'C:\Users\sanja\OneDrive - The Pennsylvania State University\Research\Data_Analysis\PlayGround\Droplets example', 
                           0.0488706004242 , 
                           intensities = ['Percentile', 85], 
                           timesteps = [1], 
                           zstacks = [1], 
                           channels = [2], 
                           windowsizes = [5], 
