...
def MSD_cal(lagttracks, msd_dimention, ref_graph, plotting): 
    '''
    ...
    MSD_error = (
        combined_lagtdata.groupby('dt')[['dy', 'dx']] #get each unique dt, and extract dx and dy
        .apply(lambda group: (group**2).sem()) #apply square elemet wise and get the std
        .reset_index() #put dt to a column
    )
    
    
    #Modify column names
    MSD_error.columns = ['dt', 'error_dy', 'error_dx']
    
    #MSD(τ) = <Δr(τ ))2> = <(ΔX(τ ))2> + <(ΔY (τ ))2> + <(ΔZ(τ ))2>
    if msd_dimention == 'x':
        MSD_data['msd']=MSD_data['mean_squared_dx'] 
        MSD_data['error'] = MSD_error['error_dx']
    elif msd_dimention == 'y':
        MSD_data['msd']=MSD_data['mean_squared_dy']
        MSD_data['error'] = MSD_error['error_dy']
