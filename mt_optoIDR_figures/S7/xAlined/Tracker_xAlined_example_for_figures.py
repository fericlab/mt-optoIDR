...
def tackdroplets(file, start_frame, end_frame, channel, p_threshold, search_range, memory, imaging_interval, annotate, filter_threshold, filetype):
    ...
    tp.plot_traj(linked, superimpose=frames[19], colorby='particle', ax=ax1, label=False)
    ax1.set_title('All tracked')
    
    ##filtering -OPTIONAL
    linked = tp.filter_stubs(linked, threshold=filter_threshold) #threshold = minimun # of frames to survive 
    tp.plot_traj(linked, superimpose=frames[1], ax=ax2)
    ...
    ax3.imshow(frames[19], cmap='inferno', vmax=2100, vmin=0)
    tp.plot_traj(linked, ax=ax3)
    
    # Apply axis formatter to convert pixels to micrometers
    pixel_to_um =0.049
    plt.gca().xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{x * pixel_to_um:.1f}'))
    plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y * pixel_to_um:.1f}'))
...
def RotateTracks(tracks, plotting):
    ...
    for i in tracks:
        ...
        frame_nms = np.array(i['frame'])
        frame_nms = frame_nms*2 #convert to seconds
        print(frame_nms)
        ...
        if plotting==True:
            ...
            ############color this according to the frame number/time
           # Example iteration numbers (this could be any array you already have)
            iteration_numbers = np.arange(len(x) - 1)  # One color per line segment, so 1 fewer than points
            
            # Create segments (each pair of points defines a line segment)
            points = np.array([x, y]).T.reshape(-1, 1, 2)
            segments = np.concatenate([points[:-1], points[1:]], axis=1)
            
            # Create colormap and normalization
            cmap = cm.get_cmap('viridis')
            norm = Normalize(vmin=np.min(frame_nms), vmax=np.max(frame_nms))
            
            # Create LineCollection
            lc = LineCollection(segments, cmap=cmap, norm=norm, array=iteration_numbers, linewidth=2)

            ax1.add_collection(lc)
            cb = plt.colorbar(lc, ax=ax1, label="Time (s)")

            
            
            
            ax1.scatter(x, y, label='Droplet locations', c=frame_nms,  cmap=cmap , alpha=0.6, marker='o')
            ax1.plot(x, y_fit, color='red', label='Linear fit')
            
            #convert px to microns
            pixel_to_um =0.049
            ax1.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{x * pixel_to_um:.1f}'))
            ax1.yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y * pixel_to_um:.1f}'))
            
            #plt.title(f'angle to x = {angle_deg}')
            ax1.set_xlabel('X position (µm)', **font_properties)
            ax1.set_ylabel('Y position (µm)', **font_properties)
            ax1.tick_params(axis='both', which='major', labelsize=14)
            
            
            # Set the number of ticks
            num_ticks = 5
            ax1.xaxis.set_major_locator(MaxNLocator(num_ticks))  # Control x-axis ticks
            ax1.yaxis.set_major_locator(MaxNLocator(num_ticks)) 
            
            ax1.legend(frameon=False, fontsize=14)
            
            
            ax1.set_xlim(1 / pixel_to_um, 3 / pixel_to_um)
            ax1.set_ylim(0 / pixel_to_um, 2.2 / pixel_to_um)
            
           
            
            ax1.invert_yaxis() #invert is neccesory 
            #plt.tight_layout()
            fig.savefig('linear fit.pdf')
            plt.show()
        ...
        if plotting==True:
            ...
            ax2.plot(coordinates[:,0], coordinates[:,1], label='original track', color='gray', alpha=0.6)
            ax2.plot(rotated_xy[:,0], rotated_xy[:,1], label=f'rotated by {angle_deg:.2f}°', color='black')
            #convert pixels to microns
            pixel_to_um =0.049
            ax2.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{x * pixel_to_um:.1f}'))
            ax2.yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y * pixel_to_um:.1f}'))
            
            
            
            # Set the number of ticks
            num_ticks = 5
            ax2.xaxis.set_major_locator(MaxNLocator(num_ticks))  # Control x-axis ticks
            ax2.yaxis.set_major_locator(MaxNLocator(num_ticks)) 
            
            
            ax2.set_xlabel('X position (µm)', **font_properties)
            ax2.set_ylabel('Y position (µm)', **font_properties)
            ax2.tick_params(axis='both', which='major', labelsize=14)
           
            ax2.legend(frameon=False, fontsize=14)
            
            
            ax2.set_xlim(1 / pixel_to_um, 3 / pixel_to_um)
            ax2.set_ylim(0 / pixel_to_um, 2.2 / pixel_to_um)
            
            ax2.invert_yaxis()
            plt.tight_layout()
            fig2.savefig('rotated_tack.pdf')
            plt.show()
...
linked = tackdroplets(file=r'C:/Users/sanja/OneDrive - The Pennsylvania State University/Research/paper#1/New_Figures/S6/xAline_example/WT.czi', 
             start_frame=53, 
             end_frame=75, 
             channel=1, 
             p_threshold=90, 
...
xAlined_data = xAlined[0]

xAlined_data.to_csv('xAlined_data.csv')
