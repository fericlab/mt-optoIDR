...
def tackdroplets(file, start_frame, end_frame, channel, p_threshold, search_range, memory, imaging_interval, annotate, filter_threshold, filetype):
    ...
    tp.plot_traj(linked, superimpose=frames[-1], colorby='particle', ax=ax1, label=False)
    ...
    ax3.imshow(frames[5], cmap='inferno', vmax=8000, vmin=0)
...
linked = tackdroplets(file=r'C:/Users/sanja/OneDrive - The Pennsylvania State University/Research/paper#1/New_Figures/Fig3/Tracking_images/Drp1k38a/Drp1k38a.czi', 
             start_frame=5, 
             end_frame=50, 
             channel=1, 
             p_threshold=50, 
             search_range=50, 
             memory=5, #Keep 0 to get a track with all frames
             imaging_interval=2, 
             annotate=False, 
             filter_threshold=10,
             filetype='CZI')
...
strt =0
endd = 45
plt.plot(time_refd[strt:endd], x_diff[strt:endd], label='ΔX', color='#7E287E', alpha=1, marker='o')
plt.plot(time_refd[strt:endd], y_diff[strt:endd], label='ΔY', color='gray', alpha=1, marker='o')
...
distance_data.to_csv('Distance_data_Drp1k38a.csv')
...
#plt.axhline(x_mean, color='#d55e00', linestyle='--')
...
tracks_list[0].to_csv('track_data_Drp1k38a.csv')
